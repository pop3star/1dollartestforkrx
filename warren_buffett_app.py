...
.badge-good   { background:#0a2d1a; color:#56d364; border:1px solid #155229; }
...
def _extract_eps_from_fs(fs: pd.DataFrame, reprt_code: str = "11011") -> dict | None:
    """
    재무제표 DataFrame에서 EPS/DPS/순이익 추출.
    reprt_code: "11011"=연간, "11014"=Q3, "11012"=반기, "11013"=Q1
    ★ thstrm_add_amount(누적금액)는 분기/반기 보고서에서만 사용.
      연간보고서(11011)에서 이 컬럼은 Q3 YTD값이나 총순이익 등 오염된 값을 가질 수 있음.
    """
    _is_annual = (reprt_code == "11011")
    ...
    _PARTIAL_PREFIXES = ("계속영업", "중단영업")

    def find(kws, exclude_partial: bool = False):
        for kw in kws:
            try:
                ...
                # [안전장치 1] '희석' 계정 배제 (희석주당이익 ≠ 기본주당이익)
                rows = rows[~rows[nm_col].astype(str).str.contains("희석", na=False, regex=False)]
                if rows.empty:
                    continue

                # [안전장치 1b] 계속영업·중단영업 계정 배제 (합계 EPS를 먼저 찾기 위함)
                if exclude_partial:
                    rows = rows[~rows[nm_col].astype(str).str.startswith(_PARTIAL_PREFIXES, na=False)]
                    if rows.empty:
                        continue
                ...
                # [안전장치 2] 분기/반기 보고서에만 누적금액(YTD) 우선 사용
                # 연간보고서(11011)에서는 add_amount가 Q3 YTD나 총순이익으로 오염될 수 있어 사용 안 함
                if not _is_annual and add_amt_col:
                    v = str(row.get(add_amt_col, "")).strip()
                    if v not in ("", "-", "―", "nan"):
                        val = v

                # [안전장치 3] 당기금액 (thstrm_amount) — 연간보고서의 1순위, 분기의 폴백
                if val is None:
                    v = str(row.get(amt_col, "")).strip()
                    if v not in ("", "-", "―", "nan"):
                        val = v
    ...
    # EPS 탐색 1순위: 합계 기본주당이익 (계속/중단영업 분리 계정 명시 배제)
    eps = find([
        "기본주당이익", "기본주당순이익", "보통주기본주당이익",
        "보통주 1주당 순이익", "주당순이익", "주당이익", "주당손익",
    ], exclude_partial=True)

    # EPS 탐색 2순위: 계속영업 + 중단영업 분리 공시된 경우 합산
    if eps is None:
        eps_cont = find([
            "계속영업기본주당이익", "계속영업기본주당순이익",
            "계속영업주당이익", "계속영업주당순이익",
            "계속영업 보통주 1주당순이익",
        ])
        eps_disc = find([
            "중단영업기본주당이익", "중단영업기본주당순이익",
            "중단영업주당이익", "중단영업주당순이익",
            "중단영업 보통주 1주당순이익",
        ])
    ...
    for reprt_code, label in attempts:
        try:
            ...
            if fs is None or (isinstance(fs, pd.DataFrame) and fs.empty):
                continue
            # reprt_code 전달: 연간(11011)은 thstrm_add_amount 사용 안 함
            result = _extract_eps_from_fs(fs, reprt_code=reprt_code)
            if result:
                return result, label
        except Exception:
            continue
...
# ════════════════════════════════════════════════════════════════
# DART 공시 기반 주식수 변동 이벤트 감지
# ════════════════════════════════════════════════════════════════

def _clean_html_to_text(html_str: str) -> str:
    """HTML/XML 태그 제거, 공백 정규화"""
...
def _parse_capital_ratio_from_doc(
    rcept_no: str, event_type: str, dart_inst, log_fn=None
) -> float | None:
    """
    DART 공시 원문에서 증자/분할/병합 비율 파싱.
    반환: 주식수 배율 (>1 = 주식수 증가, <1 = 감소). 파싱 실패 시 None.
    """
    def _log(msg):
        if log_fn:
            log_fn(msg)
    ...
    try:
        raw = dart_inst.document(rcept_no)
    except Exception as e:
        _log(f"  [DART] 문서 다운로드 실패 ({rcept_no}): {e}")
        return None
    ...
    # ── 무상증자 / 유무상증자 ────────────────────────────────────
    if event_type in ('bonus_issue', 'mixed_issue'):
        # 패턴1: "1주당 N주" or "1주당 N.N주"
        m = re.search(r'1\s*주[당대]?\s*([0-9]+\.?[0-9]*)\s*주', text)
        if m:
            per_share = float(m.group(1))
            if 0 < per_share <= 100:
                return 1.0 + per_share

        # 패턴2: "신주배정비율 N%" or "배정비율 N.N%"
        m = re.search(r'(?:신주\s*)?배정\s*비율[^0-9]*([0-9]+\.?[0-9]*)\s*%', text)
        if m:
            pct = float(m.group(1))
            if 0 < pct <= 10000:
    ...
    # ── 액면분할 ──────────────────────────────────────────────────
        ...
        # 패턴1: "분할전 N원 ... 분할후 M원"
        m = re.search(
            r'분할\s*전[^0-9]*([0-9,]+)\s*원[^0-9]*분할\s*후[^0-9]*([0-9,]+)\s*원',
            text,
        )
        if m:
            old_par = float(m.group(1).replace(',', ''))
            new_par = float(m.group(2).replace(',', ''))
            if old_par > new_par > 0:
                return old_par / new_par

        # 패턴2: "1주를 N주로 분할"
        m = re.search(r'1\s*주[를을]?\s*([0-9]+)\s*주로\s*(?:분할|액면)', text)
        if m:
            ratio = float(m.group(1))
            if 1 < ratio <= 100:
                return ratio

        # 패턴3: "N:1 분할" or "분할 N:1"
        m = re.search(r'분할[^0-9]{0,10}([0-9]+)\s*:\s*1', text)
        if m:
            ratio = float(m.group(1))
            if 1 < ratio <= 100:
                return ratio

        # 패턴4: "N원에서 M원으로" (액면가 변경)
        m = re.search(r'([0-9,]+)\s*원에서\s*([0-9,]+)\s*원으로', text)
        if m:
            old_par = float(m.group(1).replace(',', ''))
            new_par = float(m.group(2).replace(',', ''))
            if old_par > new_par > 0:
    ...
    # ── 액면병합 ──────────────────────────────────────────────────
        ...
        # 패턴1: "N주를 1주로 병합"
        m = re.search(r'([0-9]+)\s*주[를을]?\s*1\s*주로\s*병합', text)
        if m:
            ratio = float(m.group(1))
            if 1 < ratio <= 100:
                return 1.0 / ratio

        # 패턴2: 액면가 증가 ("분할전 N원 ... 분할후 M원" — 병합 시 M > N)
        m = re.search(
            r'분할\s*전[^0-9]*([0-9,]+)\s*원[^0-9]*분할\s*후[^0-9]*([0-9,]+)\s*원',
            text,
        )
        if m:
            old_par = float(m.group(1).replace(',', ''))
            new_par = float(m.group(2).replace(',', ''))
            if new_par > old_par > 0:
                return old_par / new_par  # < 1

    return None


def get_capital_change_events_from_dart(
    ticker: str,
    start_year: int,
    end_year: int,
    dart_inst,
    ...
    """
    DART 주요사항보고(kind='B')에서 유상/무상/유무상증자, 액면분할/병합 이벤트 수집.
    자회사 공시 제외:
      ① stock_code 필드가 ticker와 일치하지 않는 공시 제외
      ② report_nm에 "종속회사" / "자회사" / "피투자회사" 포함 시 제외
    반환: [{"year":int, "month":int, "event_type":str, "ratio":float|None, "desc":str}, ...]
    """
    def _log(msg):
        if log_fn:
            log_fn(msg)
    ...
    try:
        ...
        _log(f"  [DART] 주요사항보고 목록 조회 실패: {e}")
        return []

    if df is None or (isinstance(df, pd.DataFrame) and df.empty):
        return []

    # ① 자회사 공시 제외 — stock_code 필드로 필터
    if 'stock_code' in df.columns:
        df = df[df['stock_code'].astype(str).str.strip() == ticker]
    ...
    # ② 증자/분할/병합 관련 공시만 추출
    _INCLUDE_KWS = [
        "유상증자결정", "무상증자결정", "유무상증자결정",
        "주식분할결정", "액면분할결정", "주식병합결정", "액면병합결정",
    ]
    _EXCLUDE_KWS = ["종속회사", "자회사", "피투자회사"]

    include_mask = df[nm_col].str.contains('|'.join(_INCLUDE_KWS), na=False)
    exclude_mask = df[nm_col].str.contains('|'.join(_EXCLUDE_KWS), na=False)
    ...
    for _, row in df.iterrows():
        ...
        # 이벤트 타입 분류
        if '무상증자' in report_nm:
            event_type = 'bonus_issue'
        elif '유무상증자' in report_nm:
            event_type = 'mixed_issue'
        elif '유상증자' in report_nm:
            event_type = 'rights_issue'
        elif '분할' in report_nm:
            event_type = 'split'
        elif '병합' in report_nm:
            event_type = 'merge'
        else:
            continue

        _log(f"  [DART 공시] {year}.{month:02d} — {report_nm} ({rcept_no})")

        # 비율 파싱 (유상증자는 희석 계산이 복잡 → 파싱 시도만, 실패 시 None)
        ratio = None
        try:
            ratio = _parse_capital_ratio_from_doc(
                rcept_no, event_type, dart_inst, log_fn
            )
            if ratio is not None:
                _log(f"    → 비율 파싱 성공: {ratio:.4f}x ({report_nm})")
            else:
                _log(f"    → 비율 파싱 실패, EPS 기반 보완 예정")
        except Exception:
            pass
        time.sleep(0.3)
...
def adjust_eps_for_splits(
    fund_by_year: dict,
    price_by_year: dict = None,
    dart_events: list = None,   # ← NEW: DART 공시 기반 이벤트 목록
    log_fn=None,
) -> tuple[dict, list]:
    """
    연도별 EPS 변동을 감지하여 액면분할/병합/증자 보정.
    [우선순위] DART 공시 기반 이벤트 → EPS+주가 비율 기반 탐지 (미감지 연도 보완)
    [교차 검증 로직] 순이익 증감률 또는 수정주가 P/E 증감률을 사용하여 펀더멘털 변동과 주식수 변동을 구분.
    """
    ...
    # ── [1] DART 공시 기반 이벤트 우선 처리 ────────────────────────────
    # DART에서 비율이 파싱된 연도는 EPS 비율 기반 탐지를 건너뜀
    dart_handled_years: set[int] = set()

    if dart_events:
        # 같은 연도에 복수 공시가 있는 경우 배율 누적 곱셈 처리
        year_ratio_map: dict[int, float] = {}
        year_desc_map:  dict[int, str]   = {}

        for ev in dart_events:
            ev_year  = ev.get("year")
            ev_ratio = ev.get("ratio")
            ev_desc  = ev.get("desc", "")
            ev_type  = ev.get("event_type", "")

            # EPS 보정 대상 연도: fund_by_year에 존재하거나 직전/직후 연도
            if ev_year not in years:
                # 해당 공시 연도가 EPS 데이터에 없으면 가장 가까운 연도에 귀속
                later = [y for y in years if y >= ev_year]
                ev_year = later[0] if later else None
            if ev_year is None:
                continue

            # DART 비율이 성공적으로 파싱된 경우만 확정
            if ev_ratio is not None and ev_ratio > 0 and abs(ev_ratio - 1.0) > 0.05:
                existing = year_ratio_map.get(ev_year, 1.0)
                year_ratio_map[ev_year] = existing * ev_ratio
                year_desc_map[ev_year]  = ev_desc

        for ev_year, ev_ratio in year_ratio_map.items():
            ev_desc = year_desc_map.get(ev_year, "DART 공시")
            if ev_ratio >= 1.3:        # 유의미한 주식수 증가 (분할/증자)
                desc = f"{ev_year}년 주식수 변동 {ev_ratio:.2f}x [DART 공시: {ev_desc}]"
                events.append((ev_year, ev_ratio, desc))
                dart_handled_years.add(ev_year)
                _log(f"✅ {desc}")
            elif ev_ratio <= 0.77:     # 유의미한 주식수 감소 (병합)
                desc = f"{ev_year}년 주식수 변동 {ev_ratio:.3f}x [DART 공시: {ev_desc}]"
                events.append((ev_year, ev_ratio, desc))
                dart_handled_years.add(ev_year)
                _log(f"✅ {desc}")

    # ── [2] EPS+주가 비율 기반 탐지 (DART 미처리 연도만 보완) ─────────────
    for i in range(1, len(years)):
        y_prev, y_curr = years[i - 1], years[i]

        if y_curr in dart_handled_years:
            continue  # DART 공시로 이미 처리된 연도는 건너뜀
        ...
        # [교차검증 2] 수정주가 P/E 활용 (순이익 없을 때 폴백)
        ...
        if is_split:
            ratio_display = int(round(best_ratio))
            desc = f"{y_curr}년 액면분할 {ratio_display}:1 감지 (EPS {eps_prev:,.0f}→{eps_curr:,.0f})"
            if validation_msg:
                desc += f" [{validation_msg}]"
            events.append((y_curr, best_ratio, desc))
            _log(f"⚠️ {desc}")
        elif is_merge:
            ratio_display = int(round(1.0 / best_ratio))
            desc = f"{y_curr}년 액면병합 1:{ratio_display} 감지 (EPS {eps_prev:,.0f}→{eps_curr:,.0f})"
            if validation_msg:
                desc += f" [{validation_msg}]"
            events.append((y_curr, best_ratio, desc))
            _log(f"⚠️ {desc}")
    ...
    # ── 보정: 과거 EPS/DPS를 현재 주식수 기준으로 정규화 ────────────────
    adjusted = {}
    for y in years:
        divisor = 1.0
...
def analyze_stock(
    ...
    # ── DART 공시 기반 주식수 변동 이벤트 수집 ────────────────────
    dart_capital_events: list[dict] = []
    if dart_inst is not None and corp_codes is not None:
        log(f"[{ticker}] DART 공시 기반 주식수 변동 이벤트 조회 중...")
        dart_capital_events = get_capital_change_events_from_dart(
            ticker, act_start, end_year, dart_inst, corp_codes, log_fn=log
        )
        if dart_capital_events:
            log(f"[{ticker}] DART 공시 이벤트 {len(dart_capital_events)}건 감지")

    # ── 액면분할/병합/증자 EPS 보정 ───────────────────────────────
    # DART 공시 이벤트 우선 적용 → 미처리 연도는 EPS+주가 비율 기반 보완
    fund_by_year, split_events = adjust_eps_for_splits(
        fund_by_year,
        price_by_year=price_by_year,
        dart_events=dart_capital_events,
        log_fn=log,
    )
    if split_events:
        log(f"[{ticker}] 주식수 변동 {len(split_events)}건 감지 → EPS/DPS 보정 적용")
...
<b style="color:#e6edf3">📊 해석 기준 (5단계)</b>
<div class="guide-row">
  <span class="guide-badge badge-great">🏆 ≥ 2.0x &nbsp;탁월 — 탁월한 자본 배분</span>
  <span class="guide-badge badge-good">✅ 1.5 ~ 2.0x &nbsp;우수 — 높은 효율</span>
  <span class="guide-badge badge-pass">✅ 1.0 ~ 1.5x &nbsp;통과 — 효율적</span>
  <span class="guide-badge badge-warn">⚠️ 0.5 ~ 1.0x &nbsp;미흡 — 개선 필요</span>
  <span class="guide-badge badge-fail">❌ &lt; 0.5x &nbsp;실패 — 자본 배분 실패</span>
</div>
</div>""", unsafe_allow_html=True)
...
if results:
    ...
    for r in sorted(results,
        ...
        rows.append({
            ...
            "등급":       (
                "⚠️ 계산불가" if ratio is None else
                "🏆 탁월 (≥2.0)"  if ratio >= 2.0 else
                "✅ 우수 (≥1.5)"  if ratio >= 1.5 else
                "✅ 통과 (≥1.0)"  if ratio >= 1.0 else
                "⚠️ 미흡 (≥0.5)"  if ratio >= 0.5 else
                "❌ 실패 (<0.5)"
            ),
        })
    ...
    with col_l:
        ...
        if ratio is None:
            ...
            st.markdown(
                f'<div class="guide-badge badge-great" style="display:inline-block;font-size:.95em;padding:8px 16px">'
                f'🏆 탁월 &nbsp;|&nbsp; $1 비율 <b>{ratio:.2f}x</b> &nbsp;— 탁월한 자본 배분</div>',
                unsafe_allow_html=True,
            )
        elif ratio >= 1.5:
            st.markdown(
                f'<div class="guide-badge badge-good" style="display:inline-block;font-size:.95em;padding:8px 16px">'
                f'✅ 우수 &nbsp;|&nbsp; $1 비율 <b>{ratio:.2f}x</b> &nbsp;— 높은 효율</div>',
                unsafe_allow_html=True,
            )
        elif ratio >= 1.0:
            st.markdown(
                f'<div class="guide-badge badge-pass" style="display:inline-block;font-size:.95em;padding:8px 16px">'
                f'✅ 통과 &nbsp;|&nbsp; $1 비율 <b>{ratio:.2f}x</b> &nbsp;— 효율적</div>',
                unsafe_allow_html=True,
            )
        elif ratio >= 0.5:
            st.markdown(
                f'<div class="guide-badge badge-warn" style="display:inline-block;font-size:.95em;padding:8px 16px">'
                f'⚠️ 미흡 &nbsp;|&nbsp; $1 비율 <b>{ratio:.2f}x</b> &nbsp;— 개선 필요</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="guide-badge badge-fail" style="display:inline-block;font-size:.95em;padding:8px 16px">'
                f'❌ 실패 &nbsp;|&nbsp; $1 비율 <b>{ratio:.2f}x</b> &nbsp;— 자본 배분 실패</div>',
                unsafe_allow_html=True,
            )
        ...
        mc1.metric(
            "📈 주가 CAGR",
            f"{r['price_cagr']:+.1f}%" if r['price_cagr'] is not None else "N/A",
            help=f"{r['start_label']} ~ {r['end_label']} ({r['years_analyzed']}년 기준)",
        )
        mc2.metric(
            "📊 누적 수익률",
            f"{r['price_appreciation_pct']:+.1f}%" if r['price_appreciation_pct'] else "N/A",
            help=f"{r['start_price']:,.0f}원 → {r['end_price']:,.0f}원",
        )
        st.markdown(
            f"**시작가**: {r['start_price']:,.0f}원 &nbsp;→&nbsp; **현재가**: {r['end_price']:,.0f}원  \n"
            f"**주가 상승분**: {r['price_appreciation']:+,.0f}원  \n"
            f"**누적 유보EPS**: {r['total_retained_eps']:,.0f}원",
            unsafe_allow_html=True,
        )
